...
class Base extends CI_Controller {
	...
	public function Salvar(){
		...
		if($validacao){
			...
			$status = $this->usuarios_model->Inserir($contato);
	...
	public function Editar(){
		...
		$dados['contato'] = $this->usuarios_model->GetById($id);
	...
	public function Atualizar(){
		...
		if($validacao){
			...
			$status = $this->usuarios_model->Atualizar($contato['id'],$contato);
			// Checa o status da operação gravando a mensagem na seção
			if(!$status){
				$dados['contato'] = $this->usuarios_model->GetById($contato['id']);
	...
	public function Excluir($id){
		...
		$status = $this->usuarios_model->Excluir($id);
		// Checa o status da operação gravando a mensagem na seção
		if($status){
	...
	private function Validar($operacao = 'insert'){
		...
		switch($operacao){
			case 'insert':
				...
				$rules['email'] = array('trim', 'required', 'valid_email', 'is_unique[usuarios.email]');
				break;
			case 'update':
				$rules['nome'] = array('trim', 'required', 'min_length[3]');
